process MOVE_VOLT {
    ...
    files_dat=\$(shopt -s nullglob; count "\${dl_path}"/*vcs*.dat)
    files_ics=\$(shopt -s nullglob; count "\${dl_path}"/*_ics.dat)

    # Create the target directory
    targetdir="${params.vcs_dir}/${params.obsid}"
    ...
    if [[ "\$files_dat" -gt 0 && "\$files_ics" == 0 ]]; then
        ...
        find "\${dl_path}" -maxdepth 1 -name "*.dat" -exec mv -t "\${targetdir}/raw" {} +
        ...
        [[ \$files_ics -gt 0 ]] && find "\${dl_path}" -maxdepth 1 -name "*.dat" -exec mv -t "\${targetdir}/combined" {} +
    else
        exit 7
    fi
